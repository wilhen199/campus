...
df = pd.read_excel('./Files/uptime.xlsx','Hoja1')

# Guardar el DataFrame en un archivo Excel
output_file = './Results/reachable_results.xlsx'
...
def verify_device(row):
    ...
    try:
        with ConnectHandler(**device_params_local) as net_connect:
            ...
            reachable = net_connect.is_alive()
            ...
            if reachable:
                result = f"{ip_address};{expected_hostname};{result_script};{current_prompt};{uptime}"
            else:
                result = f"{ip_address};{expected_hostname};not reachable;;"            
...
header = ['ip_address', 'expected_hostname', 'result', 'prompt', 'uptime']  # Definir el encabezado del archivo
