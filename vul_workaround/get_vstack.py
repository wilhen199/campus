...
def verify_device(row):
    ...
    try:
        with ConnectHandler(**device_params_local) as net_connect:
            ...
            output = net_connect.send_command("show vstack config", expect_string=current_prompt, read_timeout=180)
            vstack = re.search(r'Role:\s+(.*)', output).group(1).strip()
            print(vstack)

            result = f"{ip_address},{expected_hostname},{current_prompt},{vstack}"
